describe('FairShare - Property & Regional Rules', () => {
  ...
  it('adjusts for Scottish regional variations (EH postcode)', () => {
    ...
    cy.get('[data-ui="regionAnnouncement"]').should('contain.text', 'Scotland');
    cy.get('[data-cy="propertyPrice-input"]').clear().type('300000').blur();
    cy.get('[data-cy="taxBand-D"]').check({ force: true });
    cy.get('[data-cy="next-button"]').click();
    
    cy.get('[data-cy="screen-4"]').should('be.visible');
    cy.get('[data-ui="totalUpfrontDisplay"]').should('contain.text', '£35,800');
  });
  ...
  it('applies surcharge for additional property purchase', () => {
    ...
    cy.get('[data-ui="totalUpfrontDisplay"]').should('contain.text', '£36,200');

    // Select "Buy-to-let" on screen 4
    cy.get('input[name="homeType"][value="second"]').check({ force: true });
    // Additional Surcharge (+3% of 300k = +9k): 36,200 + 9,000 = £45,200
    cy.get('[data-ui="totalUpfrontDisplay"]', { timeout: 10000 }).should('contain.text', '£45,200');
  });
  ...
  it('back-calculates percentages for fixed amount deposits', () => {
    ...
    cy.get('input[name="depositType"][value="amount"]').check({ force: true });
