...
4. **Build Tracking:** Increment the build number and update the build datetime in the `index.html` footer before EVERY push to GitHub. Always use **GMT Standard Time** for the timestamp. Bumping the build number must also update the static asset cache buster tokens in `index.html` (e.g., `?v=1740478179`) to the current Unix timestamp.
