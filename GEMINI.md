...
4. **Build Tracking:** Execute `npm run bump` before EVERY push to GitHub. This automatically increments the build number, updates the GMT timestamp in the `index.html` footer, and refreshes all static asset cache buster tokens (e.g., `?v=1740478179`). Every build bump MUST also be published as a corresponding GitHub Tag (e.g., `v1.1.61`).
